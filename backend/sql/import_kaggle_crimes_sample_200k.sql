...
--   mysql> SOURCE backend/sql/import_kaggle_crimes_sample_200k.sql;
--
-- Notes:
-- - Requires: `SET GLOBAL local_infile=1;` and client `--local-infile=1`
-- - Imports `data/Crimes_sample_200k.csv` (generated for quick local testing)

SET @date_fmt_primary = '%m/%d/%Y %h:%i:%s %p';
SET @date_fmt_iso = '%Y-%m-%d %H:%i:%s';
SET @date_fmt_iso_ms = '%Y-%m-%dT%H:%i:%s.%f';

LOAD DATA LOCAL INFILE 'data/Crimes_sample_200k.csv'
INTO TABLE crimes
